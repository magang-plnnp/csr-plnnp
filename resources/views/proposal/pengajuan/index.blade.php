 ...
 @section('content')
    ...
    @foreach ($proposal as $data)
        <tr>
            ...
            <td><span class="fw-normal">{{ $data->kecamatan_nama }} - {{ $data->kelurahan_nama }}</span></td>
            <td><span class="fw-normal">{{ \Carbon\Carbon::parse($data->tanggal_disposisi)->translatedFormat('d F Y') }}</span></td>
            <td><span class="fw-normal">{{ $data->nominal_pengajuan ? 'Rp' . number_format($data->nominal_pengajuan, 0, ',', '.') : '-' }}</span></td>
            ...
            <td><span class="fw-normal">{{ $data->nominal_disetujui ? 'Rp' . number_format($data->nominal_disetujui, 0, ',', '.') : '-' }}</span></td>
            ...
            <td><span class="fw-normal">{{ \Carbon\Carbon::parse($data->overdue)->translatedFormat('d F Y') }}</span></td>
            ...
            <td>
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton{{ $data->id }}">
                        <li>
                            <a href="{{ route('proposal.edit', $data->id) }}" class="dropdown-item text-primary">Edit</a>
                        </li>
...
function forceFixedColumnsBackground() {
    ...
    $('.dtfc-fixed-left.dt-head, .dt-column-left.dt-head, thead .dtfc-fixed-left, thead .dt-column-left').each(function() {
        this.style.setProperty('background-color', '#f8f9fa', 'important');
        this.style.setProperty('background', '#f8f9fa', 'important');
    });
}
